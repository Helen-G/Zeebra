...
namespace AFT.RegoV2.Infrastructure.OAuth2
{
    public class GameProviderOAuthStore : IOAuth2ClientStore
    {
        ...
        public GameProviderOAuthStore(IGameRepository repository)
        {
            _repository = repository;
        }


        public IClientDescription GetClient(string clientId)
        {
            var client = _repository.GameProviders.SingleOrDefault(x => x.AuthorizationClientId == clientId);

            if (client != null)
            {
                var oauthClient = new OAuth2Client(clientId, client.AuthorizationSecret);
                return oauthClient;
            }
            return null;
        }
