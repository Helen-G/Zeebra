...
namespace AFT.RegoV2.GameApi.Tests.Integration.Steps
{
    ...
    public class CoreGameApi : SpecFlowIntegrationTestBase
    {
        ...
        public async Task WhenICallToValidateToken()
        {
            ...
            var response =
                await JsonPost<ValidateTokenResponse>(
                    Config.GameApiUrl + "token/validate",
                    new ValidateToken
                    {
        ...
        public async Task WhenIBet(decimal amount)
        {
            ...
            var response = 
                await JsonPost<PlaceBetResponse>(Config.GameApiUrl + "bets/place", 
                        new PlaceBet
                        {
                            AuthToken = token,
        ...
        public async Task WhenIWin(decimal amount)
        {
            ...
            var response = 
                await JsonPost<PlaceBetResponse>(Config.GameApiUrl + "bets/win", 
                    new WinBet
                    {
                        AuthToken = token,
        ...
        public async Task WhenILoseTheBet()
        {
            ...
            var roundId = Get<string>(SR.roundId);
            var response = 
                await JsonPost<LoseBetResponse>(Config.GameApiUrl + "bets/lose", 
                    new LoseBet
                    {
                        AuthToken = token,
                        Transactions = new List<SettleBetTransaction>
        ...
        public async Task WhenIGetFreeBetFor(decimal amount)
        {
            ...
            var betIdsList = GetOrCreate(SR.roundIdsList, () => new List<string>());
            var response = 
                await JsonPost<FreeBetResponse>(Config.GameApiUrl + "bets/freebet", 
                    new FreeBet
                    {
                        AuthToken = token,
                        Transactions = new List<SettleBetTransaction>
        ...
        public async Task WhenISettleTheFollowingBets(Table table)
        {
            ...
            var i = 0;
            var response = 
                await JsonPost<SettleBetsResponse>(Config.GameApiUrl + "batch/bets/settle", 
                    new SettleBets
                    {
                        BatchId = NewStringId,
                        BrandKey = "138",
        ...
        public async Task WhenIGetBalance()
        {
            var token = Get<string>(SR.token);
            var balance = 
                await JsonGet<GetBalanceResponse>(Config.GameApiUrl + "players/balance?authtoken=" + token);
        ...
        public async Task WhenIGetHistory()
        {
            var token = Get<string>(SR.token);
            var history = 
                await JsonGet<BetsHistoryResponse>(Config.GameApiUrl + "bets/history?authtoken=" + token);
        ...
        public async Task WhenIAdjustTransactionWith(decimal amount)
        {
            ...
            var token = Get<string>(SR.token);
            var response = 
                await JsonPost<AdjustTransactionResponse>(Config.GameApiUrl + "transactions/adjust", 
        ...
        public async Task WhenICancelTheLastTransaction()
        {
            ...
            var response = 
                await JsonPost<CancelTransactionResponse>(Config.GameApiUrl + "transactions/cancel", 
