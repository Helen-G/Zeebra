...
Feature: ACS Soleil Game Api
	Calling ACS Soleil Game API 
...
Scenario: Get and validate authentication token
	Given I get authentication token for player "testplayer" with password "123456" for game "Roulette"
		And I get authorization token for game provider "ACS_DEV_CLIENT_ID" with secret "ACS_DEV_CLIENT_SECRET"
...
Scenario: Settle batch of bets
	...
	When I settle the following bets:
		...
		Then I will get error code "NoError"
	When I get balance
		# expect balance: 20 - 5 - 5 - 5 + 10 + 2 = 17
		Then the player's playable balance will be $17
